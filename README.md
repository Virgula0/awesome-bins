# awesome-bins

Useful static binaries for pentesting or IT stuffs.

[![Release Action](https://github.com/Virgula0/awesome-bins/actions/workflows/release.yaml/badge.svg)](https://github.com/Virgula0/awesome-bins/actions/workflows/release.yaml)
[![GitHub Release](https://img.shields.io/github/v/release/Virgula0/awesome-bins)](https://github.com/Virgula0/awesome-bins/releases)

> [!WARNING]
> This repo is experimental. It tries to release update binaries automatically but compilation directives change a lot between
> different versions. Also, the push releases is experimental and it depends by how much time compilations needs, the best way is to
> run the script locally (see the run section) otherwise rely on github releases (release section)


# Supported OS

Only linux binaries are supported at the moment, maybe in future I'll consider `Windows` and `MacOS` too.

# PRs

Pull requests are welcome but be sure to follow the same pattern of current existing modules.

# Releases

You can just check for binaries here: [https://github.com/Virgula0/awesome-bins/releases](https://github.com/Virgula0/awesome-bins/releases)

# Compile binaries locally

## Pre-requisites

General use

```bash
python3 -m venv venv
source venv/bin/activate
pip install -r requirements.txt
```

You need `docker-buildx` to run cross-platform compilation options (especially for `arm` devices)
Install via `apt` or whatever package manager available

```bash
sudo apt update
sudo apt install docker-buildx
```

Once installed, run cross platform container (only once required)

```bash
cd postgre # setup in postgre for example
make setup-docker
```

## Basic usage

```bash
python3 main.py --help
usage: main.py [-h] [--verbose | --no-verbose] [--check-version-only | --no-check-version-only] [--multi-thread | --no-multi-thread] [--single SINGLE] [--list-modules]

Awesome Bins

options:
  -h, --help            show this help message and exit
  --verbose, --no-verbose
  --check-version-only, --no-check-version-only
                        Check and update latest version of modules and exit
  --multi-thread, --no-multi-thread
  --single SINGLE       Specify single module to compile
  --list-modules        List all available modules
```

### List modules

Each of programs compiled here are colled module. The option shows all modules.

```bash
python3 main.py --list-modules
Available modules:
  - postgre
```

### Update latest versions

Update latest versions only. By default when compiling containers, the script automatically manages to update or latest versions
in `**/lts_version` but you may want to update only versions and exit without building actually something.
It is advisible to no change `**/stable_working_version` as these versions are used by the script as fallback versions whenever the
build of `lts_version` fails. This is needed because managing different versions compilation is kinda challenging and even a small variation
can distrupt the builds. Remember that most of compiled binaries contains modified `Makefile` to allow static compilations of the same.

```bash
python3 main.py --check-version-only
```

## Clean before to start

Each time is adivisble to clean old compilations with

```bash
make clean-bins
```

## Compile a single module

This is useful if you're interested in compiling a single module.

```bash
python3 main.py ----single postgre
```

Only `postgre` binaries will be compiled, you can see them at the end with `make check-bins`.

## Compile all modules (single-thread)

Run build containers one at a time:

```bash
python3 main.py --no-multi-thread
```

By default, the script runs in `multi-thread` mode.
You can find builds in `make check-bins`

## Compile all modules (multi-thread)

By default the script runs parallely more containers at a time, this can be very resource expensive:

```bash
python3 main.py
```